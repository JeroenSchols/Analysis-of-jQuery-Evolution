...
def main():
    ...
    for i1, release1 in enumerate(releases):
        for i2, release2 in enumerate(releases):
            if i1 < i2:
                i += 1
                command = f"jsinspect -r json --ignore \".*/outro\.js|.*/intro\.js\" ./jquery-data/{release1['tag']}/src ./jquery-data/{release2['tag']}/src > ../out/return/{release1['tag']}-{release2['tag']}.txt"
